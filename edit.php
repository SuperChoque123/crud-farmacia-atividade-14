<?php include "db.php"; ?>
<?php
$id = $_GET['id'] ?? 0;
$sql = "SELECT * FROM materiais WHERE id=?";
$stmt = $con->prepare($sql);
$stmt->bind_param("i", $id);
$stmt->execute();
$res = $stmt->get_result();
$material = $res->fetch_assoc();
if (!$material) {
    die("Material não encontrado!");
}

$msg = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $nome = $_POST['nome'];
    $unidade = $_POST['unidade'];
    $estoque = (int) $_POST['estoque_atual'];
    $preco = (float) $_POST['preco'];

    if ($estoque < 0 || $preco < 0) {
        $msg = "Valores inválidos!";
    } else {
        $sql = "UPDATE materiais SET nome=?, unidade=?, estoque_atual=?, preco=? WHERE id=?";
        $stmt = $con->prepare($sql);
        $stmt->bind_param("ssidi", $nome, $unidade, $estoque, $preco, $id);
        if ($stmt->execute()) {
            header("Location: index.php");
            exit;
        } else {
            $msg = "Erro ao editar.";
        }
    }
}
?>
<!DOCTYPE html>
<html lang="pt-br">
<head><meta charset="UTF-8"><title>Editar</title></head>
<body>
<h2>Editar Material</h2>
<p style="color:red;"><?= $msg ?></p>
<form method="POST">
    Nome: <input type="text" name="nome" value="<?= $material['nome'] ?>" required><br><br>
    Unidade: <input type="text" name="unidade" value="<?= $material['unidade'] ?>" required><br><br>
    Estoque Atual: <input type="number" name="estoque_atual" value="<?= $material['estoque_atual'] ?>" min="0" required><br><br>
    Preço: <input type="number" step="0.01" name="preco" value="<?= $material['preco'] ?>" min="0" required><br><br>
    <button type="submit">Atualizar</button>
</form>
<br>
<a href="index.php">Voltar</a>
</body>
</html>
